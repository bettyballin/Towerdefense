...
public abstract class Movement implements Action, IMovement {
	...
	/**
	 * the current speed, needed to calculate the distance travelled based on
	 * how much time has elapsed
	 */
	protected ISpeed entity;
	 ...
	 *            event, used to interpolate the current target position where
	 *            appropriate
	 * 
	 * @param event
	...
	public void update(GameContainer gc, StateBasedGame sb, int delta, Component event) {
		...
		// determine the target position, based on the current position,
		// movement
		// speed, orientation of the object, and time passed
		Vector2f position = getNextPosition(event.getOwnerEntity().getPosition(), entity.getSpeed()/100,
				event.getOwnerEntity().getRotation(), delta);// event.getOwnerEntity().getPosition();
