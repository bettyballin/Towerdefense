	...
	public void update(GameContainer gc, StateBasedGame sb, int delta,
		...
		if(Enemy.class.isInstance(((CollisionEvent) event).getCollidedEntity())){
			...
			if(enemy.getLife()==0) {
				...
				explosion = new ExplosionFactory(tower, shoot.getPosition().x+40, shoot.getPosition().y+50,30,50).createEntity();			
				StateBasedEntityManager.getInstance().addEntity(state, explosion);
				
				explosion = new ExplosionFactory(tower, shoot.getPosition().x+30, shoot.getPosition().y+30,100,120).createEntity();			
				StateBasedEntityManager.getInstance().addEntity(state, explosion);
